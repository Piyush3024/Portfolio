    ...
    (set, get) => ({
      ...
      fetchPostById: async (id) => {
        ...
        try {
          ...
          set({
            error: error.response?.data?.message || error.message || 'Failed to fetch post',
            isLoading: false
          });
      ...
      fetchPostBySlug: async (slug) => {
        ...
        try {
          ...
          if (response.data.success && response.data.data) {
            ...
            throw new Error(response.data.error || 'Failed to fetch post by slug');
          }
        } catch (error) {
          set({
            error: error.response?.data?.message || error.message || 'Failed to fetch post by slug',
            isLoading: false
          });
            ...
            if (get().selectedPost && get().selectedPost.comments) {
              set((state) => ({
                selectedPost: state.selectedPost ? {
                  ...state.selectedPost,
                  comments: state.selectedPost.comments ? 
                    state.selectedPost.comments.filter(comment => comment.id !== id) : 
                    []
                } : null,
                isLoading: false
              }));
